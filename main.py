...
if menu == '1':
    ...
    while not (x == 0) :
        ...
        a_file.close()
    print('end')
    print('https://discord.gg/FFqXjZgp')
    time.sleep(10)
    quit()
...
────────────────────────────────────────────────────────────────────────────────────
    ...
    def TitleUpdate(self):
        while True:
            self.SetTitle(f'[LOOT AIO Twitch Token Checker] ^| HITS: {self.hits} ^| BADS: {self.bads} ^| RETRIES: {self.retries} ^| WEBHOOK RETRIES: {self.webhook_retries} ^| THREADS: {active_count()-1}')
            sleep(0.1)
        ...
        try:
            ...
            if 'client_id' in response.text:
                ...
                if self.webhook_enable == 1:
                    self.SendWebhook('Twitch Token',token,'https://cdn.discordapp.com/attachments/860086329059442699/860207715007201280/Discord_Server_Icon_Maker_1.jpeg','https://cdn.discordapp.com/attachments/860086329059442699/860207876592893982/R2338f5ea90e75416983f8239e212cff4.png',proxy,useragent)
            else:
                self.retries += 1
